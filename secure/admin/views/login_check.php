...
if (isset($_COOKIE['admin_session']) && trim($_COOKIE['admin_session']) != '') { // user has a session already?

	require_once('../../../../dbconn_mysql.php');

	$user_session_complete_token = trim($_COOKIE['admin_session']);
	if (strpos($user_session_complete_token, ':') === false) {
		...
		if ($check_for_session->num_rows == 1) {
			...
			setcookie('admin_session', $user_session_complete_token, $new_session_key_expires, '/');
	...
	if ($check_for_user->num_rows == 1) {
		...
		$new_session_key_expires = time() + (60*60*24*30);
		setcookie('admin_session', $new_session_complete_token, $new_session_key_expires, '/');
